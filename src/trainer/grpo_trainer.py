...
class QwenGRPOTrainer(Trainer):
    def __init__(
        ...
        if self.ref_model is not None:
            # Always use evaluation_mode=True for the ref_model — it is inference-only and
            # does not need optimizer states. Using prepare_deepspeed (which enables the BF16
            # optimizer even for eval models) wastes ~28 GB of GPU on fp32 master copies.
            # ZeRO-3 sharding is handled separately in __init__ (the ref_model is loaded with
            # from_pretrained under zero.Init when zero3 is active).
    ...
    def _compute_loss(self, model, inputs):
        ...
        if getattr(self.generation_config, "lvr_steps", 0) > 0:
            ...
            _init_cache = getattr(model, "_get_initial_cache_position", None)
    ...
    def _replay_lvr_to_collect_states(
        ...
        model_kwargs = {}
        try:
            model_kwargs = self.model._get_initial_cache_position(prompt_ids.shape[1], prompt_ids.device, model_kwargs)
        except TypeError:
